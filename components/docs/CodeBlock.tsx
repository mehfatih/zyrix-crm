...
export function useCopyButtons(scopeRef: React.RefObject<HTMLElement | null>) {
  ...
  useEffect(() => {
    ...
    pres.forEach((pre) => {
      ...
      btn.className =
        "absolute top-2 end-2 inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-white/90 border border-cyan-100 rounded-md text-slate-600 hover:text-cyan-700 hover:border-cyan-300 transition-colors";
      btn.innerHTML =
  ...
  return (
    <div className="relative my-4">
      <pre
        ...
        className="bg-cyan-50 border border-cyan-100 rounded-xl p-4 overflow-x-auto text-sm"
      >
      ...
      <button
        ...
        className="absolute top-2 end-2 inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-white/90 border border-cyan-100 rounded-md text-slate-600 hover:text-cyan-700"
      >

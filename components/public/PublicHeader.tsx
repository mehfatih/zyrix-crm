    ...
    >
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {/* Compact language switcher with icon + code */}
          <div className="relative" ref={langRef}>
            <button
              onClick={() => setLangOpen(!langOpen)}
              className={`flex items-center gap-1.5 px-2.5 py-2 text-xs font-bold text-slate-600 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 rounded-lg transition-colors`}
            ...
            {langOpen && (
              <div className="absolute top-full mt-1 ltr:right-0 rtl:left-0 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-[90px] z-10">
                {LOCALES.map((loc) => (
                  <button
                    key={loc.code}
                    onClick={() => switchLocale(loc.code)}
                    className={`w-full text-left rtl:text-right px-3 py-2 text-xs font-bold transition-colors ${
                      locale === loc.code
                        ? `${accent.text} bg-slate-50`
